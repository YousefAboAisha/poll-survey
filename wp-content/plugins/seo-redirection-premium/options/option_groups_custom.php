...
//- Add - update section ---------------------------------------------
if($request->get('add')!='' || $request->get('edit','int')!=='') {
    $default_name="";
...
<h4><?php _e('Add Custom Group','wsr')?></h4>
<hr />
<form action="<?php echo $request->get_current_parameters(array("add","edit"));?>" method="post" class="form-horizontal"
    role="form" data-toggle="validator">
    <div class="form-group">
        <label class="control-label col-sm-2" for="name"><?php _e('Group Name:','wsr')?></label>
        <div class="col-sm-5">
            <input type="text" class="form-control" id="name" name="name" value="<?php echo $default_name; ?>"
                placeholder="Group Name" data-error="<?php _e('address is invalid','wsr')?>"
                title="<?php _e('Input Group Name!','wsr')?>" required autofocus>
        </div>
    </div>

    <div class="form-group">
        <div class="col-sm-offset-2 col-sm-5">
            <button type="submit" name="save" class="btn btn-default btn-sm"><span
                    class="glyphicon glyphicon-floppy-disk"></span> <?php _e('Save','wsr')?></button> <a
                href="<?php echo $request->get_current_parameters(array("add","edit"));?>"
...
//- Delete section ---------------------------------------------
}else if($request->get('del','int')!==''){
?>

<h4><?php _e('Delete Custom Group','wsr') ?></h4>
<hr />
<form action="<?php echo $request->get_current_parameters(array("add","edit","del"));?>" method="post"
    class="form-horizontal" role="form" data-toggle="validator">
    <div class="container">
        <p><?php _e('Please choose one of the following option to decide what to do with the redirects that belong the the group you want to delete :','wsr');?>
        </p>
        <div class="radio">
            <label><input type="radio" value="move_all" name="del_option"
                    checked><?php _e('Move all redirects inside this group to: ','wsr');?></label> <?php
        ...
        <div class="radio">
            <label><input type="radio" value="del_all"
                    name="del_option"><?php _e(' Delete the group and all redirects inside it.','wsr');?></label>
        </div>
        <br />
        <div class="form-group">
            <div class="col-sm-offset-2 col-sm-5">
                <input type="hidden" name="del_id" value="<?php echo $request->get('del'); ?>" />
                <button type="submit" name="Delete" value="Delete" class="btn btn-danger btn-sm"><span
                        class="glyphicon glyphicon-remove"></span> <?php _e('Delete','wsr'); ?></button> <a
                    href="<?php echo $request->get_current_parameters(array("add","edit","del"));?>"
                    class="btn btn-default btn-sm"><span class="fa fa-undo"></span> <?php _e('Cancel','wsr');?></a>
            </div>
        </div>
        <br />
...
<div class="form-group">
    <div class="col-sm-8">
        <table class="table table-bordered table-hover table-striped">
            ...
            <?php
                ...
                foreach ( $system_groups as $group ) {
                    ?>
            <tr>
                <td class="toolcell"><a class="btn btn-danger btn-xs tool"
                        href="<?php echo $request->get_current_parameters();?>&del=<?php echo $group->ID ;?>"><span
                            aria-hidden="true" class="fa fa-trash"></span></a></td>
                <td class="toolcell"><a class="btn btn-primary btn-xs tool"
                        href="<?php echo $request->get_current_parameters();?>&edit=<?php echo $group->ID ;?>"><span
                            aria-hidden="true" class="fa fa-pencil"></span></a></td>
                <td><?php echo $group->group_title ;?></td>
                <td width="100" align="center"><a
                        href="<?php echo $request->get_current_parameters(array("add","edit","grpID","redirect_manager_tab"));?>&redirect_manager_tab=redirects&grpID=<?php echo $group->ID ;?>"><?php
        ...
        <a href="<?php echo $request->get_current_parameters();?>&add=1" class="btn btn-default btn-sm"><i
                class="fa fa-plus"></i> <?php _e('Add New Group','wsr') ?></a>
        <br /><br />
    </div>
</div>
<?php
