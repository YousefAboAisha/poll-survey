...
if (!class_exists('SR_redirect_manager')) {
    ...
    class SR_redirect_manager
    {
           ...
           //echo "-" . $url_enable_lan . "_ $default_language";
            ...
            if (SRP_PLUGIN::get_options()->read_option_value('plugin_status') != 0) { // not disabled
                ...
                if (current_user_can('manage_options') == 1 && SRP_PLUGIN::get_options()->read_option_value('plugin_status') == 2) { // if disable for admin and the user is admin
					...
					//check if never redirect search results is enabled
					
					if(SRP_PLUGIN::get_options()->read_option_value('never_redirect_search_res') == 1 && isset($_GET['s']))
					{
                    ...
                    if (is_singular() && isset($post) && $post->ID > 0) {

                        $post_cache_result = $SR_redirect_cache->redirect_cached($post->ID);
                    }
                    $check_return = SRP_buddy_press_check_locking();
                    if ($check_return) {
                        return;
                    }
                    ...
                    $query_red = $wpdb->prepare("select * from $table_name where enabled=1 and cat='link' and blog=%d and regex='' and ( redirect_from = %s OR redirect_from = %s or redirect_from = %s)", get_current_blog_id(), $permalink, $permalink_alternative, $permalink_alternative.'/');
        ...
        public static function make_redirect($redirect_from, $redirect_to, $redirect_type, $src, $src_id, $obj = null, $url_enable_lan = 0)
        {
            ...
            $option_url_exclude = get_option('list_of_excluded');
            $option_url_exclude = explode(PHP_EOL, $option_url_exclude);
            if(in_array(trim(home_url().$redirect_from,' '),$option_url_exclude)){
                return;
            }
        ...
        public static function get_alternative_permalink()
        {
			...
			return str_replace(home_url(), '', $permalink_alternative); 
